...
class XTreeBehavior extends CActiveRecordBehavior
{
  ...
  public $treeExpandedMethod=null;

  public $treeNodeSelectedMethod=null;
	...
	public function getRoots()
	{
        ...
        $criteria->select = 'id';
        $nodes = $owner->findAll($criteria);
        $exist_ids = CHtml::listData($nodes, 'id', 'id');
        $criteria = new CDbCriteria();
        $criteria->addNotInCondition('parent_id', $exist_ids);
	 ...
	 */
	public function getTreeItems($id=null,$showRoot=true) 		//MAIN
	{
		$owner=$this->getOwner();
		$rootId=($id===null) ? $this->getRootId() : $id;
		$items=array();
		...
		else
		{
			...
			if($model===null)
				...
				$items[]=$model->getTreeSubItemsFast();
		}
		return $items;
	}
     ...
     * add by chenwang
	 */
    protected function getTreeSubItemsFast() //MAIN SELECTED
    {
		$owner=$this->getOwner();
		$subItems=array();
        $criteria = new CDbCriteria;
        $criteria->order = 'parent_id';
        $owner->setScenario('display');
        $subs = $owner->findAll($criteria);
        $models = array();
        $parents = array();
        $childCount = array();
        foreach ($subs as $sub) {
            $models[$sub->id] = $sub;
            $parents[$sub->id] = $sub->parent_id;
        }
        foreach ($subs as $sub) {
            if (!$sub->parent_id || !(isset($models[$sub->parent_id]))) {
                continue;
            }
            if (isset($childCount[$sub->parent_id])) {
                $childCount[$sub->parent_id] ++;
                $models[$sub->parent_id]->childCount = $childCount[$sub->parent_id];
            } else {
                $models[$sub->parent_id]->childCount = 1;
                $childCount[$sub->parent_id] = 1;
            }
        }
        $items = array();
        if ($this->treeNodeSelectedMethod) {
            $selected = $owner->{$this->treeNodeSelectedMethod}();
        } else {
            $selected = null;
        }
        foreach ($models as $model) {
            $item = $this->formatTreeItemFast($model, isset($childCount[$model->id]), $selected);
            $items[$model->id] = $item;
        }
        usort($models, function ($a, $b) { //sort for user
        ...
        foreach ($models as $model) {
            $items[$model->parent_id]['children'][$model->name] = $items[$model->id];
            if ($selected !==null && $selected->id === $model->id) {
                $items[$model->id]['expanded'] = 1;
                $items[$model->id]['htmlOptions'] = array('style'=>'display:block');
                $parent_id = $model->parent_id;
                $i = 0;
                while ($parent_id) {
                    $items[$parent_id]['expanded'] = 1;
                    $items[$parent_id]['htmlOptions'] = array();
     ...
     * add by chenwang
	 */
	public function formatTreeItemFast($model, $hasChildren, $selected=null)
	{
		...
		if($this->treeUrlMethod!==null)
			...
			$url='#';
        $htmlOptions = array('id'=>$model->getAttribute($this->id));
        if ($selected && $selected->id === $model->id) {
            $htmlOptions['class'] = 'selected';
        }
		return array(
            'text'=>CHtml::link($label, $url, $htmlOptions),
            'id'=>$model->getAttribute($this->id),
            'hasChildren'=>$hasChildren,
            'expanded'=>$model->{$this->treeExpandedMethod}(),
            'htmlOptions' => $selected ? array('style'=>'display:none') : array(),
        );
    }
	 ...
	 * @return array of tree item formatted for CTreeview widget
	 */
	public function formatTreeItem($model)
	{
		if($this->treeLabelMethod!==null)
			$label=$model->{$this->treeLabelMethod}();
		else

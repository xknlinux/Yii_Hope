...
EOD;
	...
	public static function checkBoxDivList($name,$select,$data,$htmlOptions=array())
	{
		$template=isset($htmlOptions['template'])?$htmlOptions['template']:'{input} {label}';
		...
		$labelOptions=isset($htmlOptions['labelOptions'])?$htmlOptions['labelOptions']:array('style'=>'text-align:left;display:inline;float:none');
		unset($htmlOptions['labelOptions']);
		...
		foreach($data as $value=>$label)
		{
			...
			$option=self::checkBox($name,$checked,$htmlOptions);
			$label=self::label($label,$htmlOptions['id'],$labelOptions);
			$items[]="<li class=checkboxlistli>".strtr($template,array('{input}'=>$option,'{label}'=>$label))."</li>";
		}
...
EOD;
        ...
        $cs=Yii::app()->getClientScript();
        $cs->registerCss('checkboxlist', '.checkboxlistli {list-style:none;width:100%;} .checkboxlist{padding:0px;}');
        $cs->registerCss('checkboxlistdiv', '.checkboxlistdiv {overflow-y:scroll; text-align:left;height:100px;}');

        $divHtmlOptions['class'] = 'checkboxlistdiv view';
        return self::tag("div", $divHtmlOptions, self::tag('ul', array('class'=>'checkboxlist'), implode($separator,$items)));
	}
	/**
	...
	public static function activeCheckBoxDivList($model,$attribute,$data,$htmlOptions=array())
	{
		self::resolveNameID($model,$attribute,$htmlOptions);
		$selection=self::resolveValue($model,$attribute);
		if($model->hasErrors($attribute))
		...
		return $hidden . self::checkBoxDivList($name,$selection,$data,$htmlOptions);
	}
